 ...
 * 或者您也可以通过上述的 startPublishing() 接口来指定当前用户的直播 CDN 地址。
 *
 * 一个直播间中可能有不止一位主播，而且每个主播都有自己的音视频流，但对于 CDN 观众来说，他们只需要一路直播流，
 * 所以您需要将多路音视频流混成一路标准的直播流，这就需要混流转码。
 *
 * setMixTranscodingConfig() 会向腾讯云的转码服务器发送一条指令，目的是将房间里的多路画面叠加到一路画面上,
 * 您可以通过 TRTCTranscodingConfig 参数来调整每一路画面的位置以及最终输出的画面质量。
 *
 * <pre>
 * 【画面1】=> 解码 => =>
 *                         \
 ...
 * 示例代码：我们在 Demo 中增加了该功能的体验入口，您可以在“设置”面板中的“混流”选项卡中体验到该功能。
 *
 * @param config 请参考 TRTCCloudDef.h 中关于 TRTCTranscodingConfig 的介绍。如果传入 nil 则取消云端混流转码。
 * @note 关于云端混流的注意事项：
 *       - 云端转码会引入一定的 CDN 观看延时，大概会增加1 - 2秒。
 *       - 调用该函数的用户，会将多路画面混合到一路 [CDN 地址](https://cloud.tencent.com/document/product/647/16826) 上。
 */
